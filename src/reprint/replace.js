...
files.forEach(p => {
  ...
  const replace = content.replace(/!\[]\(https:\/\/user-gold-cdn.xitu.io\/(.*)\)/g, (value) => {
    const res = value.split(/!\[]\((.*)\)/g)[1].split(' ')[0]
    const tmp = res.replace('https://user-gold-cdn.xitu.io/', 'https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/')
